...
> 当前状态：工程骨架、Legacy 适配层、本地 SQLite、Koa API、定时健康检测、SSE、正式 Web 看板、手动操作和自动切换闭环已完成。页面已支持自动切换确认、实时任务跟踪、冷却提示和异常后的安全停用；下一阶段进行终端运行版 MVP 总验收。
...
并将上次进程遗留的排队中或运行中任务标记为“服务重启中断”，不会自动重放配置修改。
...
自动切换默认关闭，只能在当前订阅已经受管锁定后由用户确认开启，并绑定当前订阅 UID。手动“立即检测”和定时检测共用同一状态机；真正进入自动处理时会创建可追踪的 `auto_switch` 任务。成功、无不同候选及可安全恢复的失败都会从任务终态开始进入冷却；恢复失败、恢复状态未知或服务重启中断时会自动关闭开关并清空绑定 UID，等待用户检查后重新启用。
